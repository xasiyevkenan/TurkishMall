...
import { CommonDimensions } from './dimension';

const { windowHeight: deviceHeight, windowWidth: deviceWidth } = CommonDimensions;

// Constants for design layout
const layoutWidth = 375;
const layoutHeight = 812;
const padding: number = 24;

// Calculate scale factors based on device and design layout
const widthScaleFactor = deviceWidth / layoutWidth;
const heightScaleFactor = deviceHeight / layoutHeight;

// Font scaling based on device's font scale
...
const fontScaleValue = (fontScales.find(scale => maxFont <= scale.limit) || { value: 0 }).value;
...
/**
 * Main function to normalize sizes based on the type of normalization required.
 * @param func - The type of normalization function (width, height, font, etc.).
 * @param px - The size value to be normalized.
 * @returns The normalized pixel value based on the specified function type.
 */
